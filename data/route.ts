...
export async function GET(request: NextRequest) {
  try {
    ...
    const [topupsRes, alertsRes, bankPaymentsRes] = await Promise.all([
      supabaseAdmin
        .from("wallet_topups")
        .select("*")
    ...
    const rawTopups = topupsRes.error
      ? []
      : (((topupsRes.data || []) as AnyRow[]).filter(isBrebTopup).map(normalizeTopup));

    if (topupsRes.error) {
      partialErrors.push(`wallet_topups: ${topupsRes.error.message}`);
    }

    const alerts = alertsRes.error ? [] : alertsRes.data || [];

    if (alertsRes.error) {
      partialErrors.push(`wallet_topup_alerts: ${alertsRes.error.message}`);
    }

    const bankPayments = bankPaymentsRes.error
      ? []
      : (((bankPaymentsRes.data || []) as AnyRow[])
          .map(normalizeBankPayment)
          .sort(sortByNewest)
          .slice(0, 500));

    if (bankPaymentsRes.error) {
      partialErrors.push(`bank_payment_notifications: ${bankPaymentsRes.error.message}`);
    }

    const userIds = Array.from(
      new Set(rawTopups.map((topup) => topup.user_id).filter(Boolean))
    ) as string[];
    ...
    if (userIds.length > 0) {
      ...
      if (error) {
        partialErrors.push(`profiles: ${error.message}`);
      } else {
    ...
    const topups = rawTopups.map((topup) => {
    ...
    return NextResponse.json({
      ...
      counts: {
        ...
        availableBankPayments: bankPayments.filter((payment) => !payment.is_used).length,
      },
    });
  } catch (error) {
