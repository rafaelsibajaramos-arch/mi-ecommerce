...
export default function AdminWalletPage() {
  ...
  const loadTransactions = useCallback(async () => {
    ...
    const manualTransactions = rawTransactions.filter((transaction) => {
      const text = `${transaction.note || ""} ${transaction.description || ""}`
        .toLowerCase()
        .normalize("NFKD")
        .replace(/[\u0300-\u036f]/g, "");

      const looksAutomaticTopup =
      ...
      return !looksAutomaticTopup;
    });

    const userIds = Array.from(
      new Set(manualTransactions.map((item) => item.user_id).filter(Boolean))
    ) as string[];
    ...
    const formatted = manualTransactions
      .map((transaction) => {
  ...
  async function handleMovement(e: FormEvent) {
    ...
    setBanner({
      ...
      text:
        movementType === "credit"
          ? `Se acreditó ${formatMoney(numericAmount)} al correo ${profile.email || "sin correo"
          }.`
          : `Se descontó ${formatMoney(numericAmount)} al correo ${profile.email || "sin correo"
          }.`,
    });
  ...
  async function handleReverseRecharge(transaction: FormattedTransaction) {
    ...
    if (transaction.source === "bank_transfer") {
      setBanner({
        kind: "error",
        text: "Las recargas automáticas por Bre-B / Llaves se gestionan desde /admin/recargas-automaticas.",
      });
      return;
    }
                ...
                {clientPaginationItems.map((item, index) =>
                  item === "..." ? (
                    ...
                    <button
                      ...
                      className={`flex h-11 min-w-[44px] items-center justify-center rounded-2xl border px-3 text-sm font-semibold transition ${effectiveClientPage === item
      ...
      >
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Aquí solo se muestran recargas y reversas manuales. Las automáticas Bre-B / Llaves se gestionan aparte en /admin/recargas-automaticas.
            </p>
          </div>
                ...
                {rechargePaginationItems.map((item, index) =>
                  item === "..." ? (
                    ...
                    <button
                      ...
                      className={`flex h-11 min-w-[44px] items-center justify-center rounded-2xl border px-3 text-sm font-semibold transition ${effectiveRechargePage === item
                          ? "border-slate-900 bg-slate-900 text-white"
                          : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
                        }`}
                    >
                  ...
                  <div key={transaction.id} className="px-5 py-5 md:px-6">
                    <div className="grid gap-4 md:grid-cols-[1.2fr_0.8fr_0.8fr_1fr] md:items-center">
                      ...
                      <div>
                        ...
                        <p
                          className={`text-sm font-bold ${normalizeType(transaction.type) === "credit"
                              ? "text-emerald-600"
                              : "text-rose-600"
                            }`}
                        >
                  ...
                  {transactionPaginationItems.map((item, index) =>
                    item === "..." ? (
                      ...
                      <button
                        ...
                        className={`flex h-11 min-w-[44px] items-center justify-center rounded-2xl border px-3 text-sm font-semibold transition ${effectiveTransactionPage === item
                            ? "border-slate-900 bg-slate-900 text-white"
                            : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
                          }`}
                      >
              ...
              <div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <button
                    ...
                    className={`rounded-2xl border px-4 py-3 text-sm font-semibold transition ${movementType === "credit"
                  ...
                  <button
                    ...
                    className={`rounded-2xl border px-4 py-3 text-sm font-semibold transition ${movementType === "debit"
