...
function getRuntimeStatus(row: AnyRow) {
  const status = String(row.status || "ACTIVE").toUpperCase();
  const now = Date.now();
  const startsAt = row.starts_at ? new Date(String(row.starts_at)).getTime() : 0;
  ...
  if (endsAt && Number.isFinite(endsAt) && endsAt < now) return "EXPIRED";
  return "ACTIVE";
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const counts = promotions.reduce(
      (acc, promotion) => {
        const runtime = getRuntimeStatus(promotion as AnyRow);
        acc.total += 1;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === "delete") {
      if (!id) return jsonError("Falta el id de la promoción.");
      const { error } = await supabaseAdmin.from("wallet_topup_promotions").delete().eq("id", id);
      if (error) return jsonError(error.message, 500);
